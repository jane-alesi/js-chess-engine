...
const mockDOM = () => {
    global.document = {
        ...
        querySelectorAll: jest.fn(() => Array(64).fill().map(() => ({
            classList: {
                add: jest.fn(),
                remove: jest.fn()
            }
...
describe('Game Class - Issue #14 Validation', () => {
    ...
    describe('Input Handling - Acceptance Criteria #4', () => {
        test('should set up click event listener on board container', () => {
            ...
            expect(mockContainer.addEventListener).toHaveBeenCalledWith('click', game.handleSquareClick);
        });
